...
int main( int argc, char* args[] )
{
    ...
    while(true)
    {
        ...
        {
            rect_character.y-=currentKeyStates[SDL_SCANCODE_LSHIFT]?2:1;
        }
        ...
        {
            rect_character.x+=currentKeyStates[SDL_SCANCODE_LSHIFT]?2:1;
        }
        ...
        if(rect_character.y>=218){
            rect_character.y-=currentKeyStates[SDL_SCANCODE_LSHIFT]?2:1;
        }
        if(rect_character.x<=0){
            rect_character.x+=currentKeyStates[SDL_SCANCODE_LSHIFT]?2:1;
        }
        if(rect_character.x>=468){
            rect_character.x-=currentKeyStates[SDL_SCANCODE_LSHIFT]?2:1;
        }
        ...
        if(frame%15==0 && currentKeyStates[SDL_SCANCODE_SPACE]){
            ...
            i++)
        {
            ...
            bullet_actual->x+=5;
            if(bullet_actual->x>500 || bullet_actual->x<0)
            {
